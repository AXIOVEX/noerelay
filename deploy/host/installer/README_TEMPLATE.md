...
| **Master config** | All llama-server runtime settings in one place; the CLI's `llm start` reads this. | `llama.yaml` |
...
├── llama.yaml                      # master llama-server config (all server settings)
└── README.md                       # this file
...
### 5b. LLM server settings — `llama.yaml` (master config)

All llama-server runtime settings live in the **master config**
`<install_dir>/llama.yaml` (NR-LLM-005). The provisioner generates it with
the detected values, and `noerelay llm start` builds the server invocation
from it — edit the YAML, then restart:
...
server:
 ...
 host: 127.0.0.1
 port: 8080
 n_gpu_layers: __NGUPLAYER__
 split_mode: none            # none | layer | row
 tensor_split: '__SPLIT__'   # relative weight per GPU (multi-GPU only)
 ctx_size: __CTX__
...
| `n_gpu_layers` | Layers to offload to the accelerator (999 = "all", 0 = CPU-only). |
| `split_mode` | `layer` splits by layer across GPUs (CUDA multi-GPU only). |
| `tensor_split` | Relative weight per GPU, in CUDA order (multi-GPU only). |
| `ctx_size` | Context window (tokens). |
| `parallel` | Concurrent request slots. |

Unknown keys are rejected at start, so a typo fails fast instead of silently
changing behaviour.

**After editing, restart the server** (`noerelay llm stop --dir <install_dir>`,
then `noerelay llm start --dir <install_dir>`).
